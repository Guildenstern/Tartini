...
namespace {
QToolButton* toolButton(const QIcon & icon, const QString & textLabel, const QString & statusTip, QObject * receiver, const char * slot, QWidget * parent, const char * /*name*/) {
    QToolButton *t = new QToolButton(parent);
    t->setIcon(icon);
    t->setText(textLabel);
    t->setStatusTip(statusTip);
    if (receiver) {
        t->connect(t, SIGNAL(triggered(QAction)), receiver, slot);
    }
    return t;
}
}
  ...
  : QMainWindow()
{
  ...
  beginningButton->setIcon(QIcon(beginning32x32_xpm));
  beginningButton->setText("Beginning");
  beginningButton->setToolTip("Rewind to the beginning");
  ...
  rewindButton->setIcon(QIcon(rewind32x32_xpm));
  ...
  fastforwardButton->setIcon(QIcon(fastforward32x32_xpm));
  ...
  autoFollowIconSet->addPixmap(QPixmap(autofollow32x32_xpm));
  QToolButton *autoFollowButton = toolButton(*autoFollowIconSet, tr("Auto Follow"), tr("Moves the view up and down automaticlly with the active channel when playing or recording"), NULL, NULL, analysisToolBar, tr("autoFollow"));
  ...
  QToolButton *backgroundShadingButton = toolButton(*backgroundShadingIconSet, "Background Shading", "Toggle background shading on/off", NULL, NULL, analysisToolBar, "backgroundShading");
...
//  timeSlider->setMargins(2, 2);
  ...
  noteLabel = new MyLabel("Note: 9999", statusBar());
  ...
  chunkLabel = new MyLabel("Chunk: 999999", statusBar());
  ...
  timeLabel = new MyLabel("Time: -00:00.000", statusBar());
...
{
  if(e->type() == AudioThread::UPDATE_FAST ||
          e->type() == AudioThread::UPDATE_SLOW) {
    ...
    gdata->need_update = false;
	  return true;
  } else if(e->type() == AudioThread::SOUND_STARTED) {
  ...
  } else if(e->type() == AudioThread::SOUND_STOPPED) {
  ...
  } else if(e->type() == AudioThread::SETTINGS_CHANGED) {
...
*/
  ...
  QString lastFolder = QDir::convertSeparators(gdata->qsettings->value("Dialogs/openFilesFolder", QDir::currentPath()).toString());
...
void MainWindow::openFile(const char *filename)
{
  ...
  QApplication::postEvent(gdata->mainWindow, new QEvent(AudioThread::UPDATE_SLOW));
  gdata->view->doUpdate();
}
...
void MainWindow::openRecord(bool andPlay)
{
  ...
  SoundFile *newSoundFile = new SoundFile();
  QString newFilename(fileInfo.absoluteFilePath().toLatin1());
//#ifdef WINDOWS
//  newFilename.replace(QChar('/'), QChar('\\'));
//#endif
  newFilename = QDir::convertSeparators(newFilename);
  if(!newSoundFile->openWrite(newFilename.toLocal8Bit().data(), rate, channels, bits, windowSize, stepSize)) {
  ...
  if(!gdata->openPlayRecord(newSoundFile, playSoundFile)) {
    ...
    QMessageBox::warning(gdata->mainWindow, "Error", QString("Error opening sound device for recording"), QMessageBox::Ok, QMessageBox::NoButton);
    return;
  }
  gdata->addFileToList(newSoundFile);
...
void MainWindow::windowMenuAboutToShow()
{
  ...
  for ( int i = 0; i < int(windows.count()); ++i ) {
      action = new QAction(windows.at(i)->windowTitle(), this);
      connect(action, SIGNAL(triggered()), this, SLOT(windowMenuActivated(int)));
      action->setData(i);
      action->setChecked(theWorkspace->activeWindow() == windows.at(i) );
  }
  
  windowMenu->addSeparator();
  action = new QAction("&Cascade", theWorkspace);
  connect(action, SIGNAL(triggered()), theWorkspace, SLOT( cascade() ));
  action->setEnabled(windows.isEmpty());
  action = new QAction("Close &All", theWorkspace);
  connect(action, SIGNAL(triggered()), theWorkspace, SLOT( closeAllWidgets() ));
  action->setEnabled(windows.isEmpty());
}
...
TartiniDialog::TartiniDialog(QWidget *parent) : QDialog(parent)
{
  ...
  tartiniPicture->setPalette(palet);
  ...
  tartiniTextEdit->setAcceptRichText(true);
  ...
  connect(GPLButton, SIGNAL(clicked()), gdata->mainWindow, SLOT(aboutGPL()));
}
